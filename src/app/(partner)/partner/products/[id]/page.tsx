...
export default async function PreviewProductPage({ params }: Props) {
  ...
  return (
    <div className="space-y-6 max-w-5xl mx-auto pb-12">
      ...
      <div className="bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800 rounded-xl p-4 flex gap-3 text-sm text-yellow-800 dark:text-yellow-200">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        {/* Left: Image */}
        <div className="bg-white dark:bg-gray-900 rounded-3xl overflow-hidden shadow-sm border border-[#FDF2F8] dark:border-gray-800 aspect-[3/4]">
        ...
        <div className="space-y-6">
          <div>
            <div className="text-sm font-semibold text-[#EC4899] mb-2">{product.categoryRel?.name || product.category}</div>
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {product.sizes && product.sizes.length > 0 ? product.sizes.map((size: string) => (
                    <span key={size} className="px-3 py-1 border border-gray-200 dark:border-gray-700 text-sm rounded-md font-medium">{size}</span>
                  )) : <span className="text-sm text-gray-400">-</span>}
                </div>
              </div>
            </div>
          ...
          <div className="pt-6 border-t border-gray-100 dark:border-gray-800">
            ...
            <ul className="space-y-2 text-sm">
              ...
              <li className="flex justify-between"><span className="text-gray-500">Status</span> <span className="font-medium text-[#EC4899]">{product.productStatus}</span></li>
      ...
      <Card className="mt-8 p-6 bg-gradient-to-tr from-[#FFF7FB] to-white dark:from-[#EC4899]/5 dark:to-transparent border-[#FDF2F8] dark:border-gray-800">
